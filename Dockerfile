...
WORKDIR /src
COPY ["OgrenciBilgiSistemi/OgrenciBilgiSistemi.csproj", "OgrenciBilgiSistemi/"]
RUN dotnet restore "./OgrenciBilgiSistemi/OgrenciBilgiSistemi.csproj"
COPY . .
WORKDIR "/src/OgrenciBilgiSistemi"
RUN dotnet build "./OgrenciBilgiSistemi.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Bu aşama, son aşamaya kopyalanacak hizmet projesini yayımlamak için kullanılır
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./OgrenciBilgiSistemi.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "OgrenciBilgiSistemi.dll"]
